...
| **M3** | `feat/facility-definitions` | 9+1 `FacilityDefinition` SOs; multiplier hooks into wallet/loot/detection/shakedown (even while only Dev's scene exists) | ✅ 7/15 — SOs in `Resources/Facilities`; loot/detection/shakedown/arrival-affinity hooks live. **7/17:** economy multipliers **consumed** — trade/source × `tradePriceMult`, bribes × `bribeCostMult`, stipend/favors × `cashIncomeMult` ([[Loot & Economy]]) |
