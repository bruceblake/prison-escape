...
- **New schedule (implemented 7/15/2026):** the day is a **13-phase loop** ([[Time & Schedule]]); `PrisonRoutineLabels.FormatPhaseTitle` supplies `WORK / PROGRAMS`, `MIDDAY COUNT`, `EVENING COUNT` alongside the existing labels, and **every HUD (legacy included) now delegates to it** — the duplicate per-widget `FormatEvent` switches were removed. `DailyRoutineBarUI` reads segments straight from the schedule asset, so the 13 phases render without changes.
