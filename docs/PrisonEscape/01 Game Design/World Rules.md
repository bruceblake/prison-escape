...
22. **Crossing the outer boundary = escaped** — end screen with run stats, any route, any time ([[Escape Completion System]]). *As implemented (v1) this is a "you're free" win screen; under the career ladder it becomes caught-and-transferred — see rules 27–33.*
...
## The career *(specced — [[Prison Career Ladder]]; not yet in code)*
...
29. **County only:** serving the full sentence (default 7 in-game days) also graduates you to State Minimum — the one escape-free promotion.
...
32. **Locked facilities are black silhouettes** on the prison select — visible, never enterable. The current Minimum-Security map is the **Dev Sandbox**, off the career path.
33. **Escaping Federal ADX = career win** — the world flags won and stays playable.
