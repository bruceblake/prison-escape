...
3. **Social polish** — install `Resources/Social/` assets; fuller per-guard Trust→detection ([[Social & Reputation]])
4. **Career hub playtest smoke** — MainMenu worlds → County/Dev → Talk / dossier / transfer ceremony

## Next

5. **Front-entrance escape route** — Main Security bypass (disguise? keycard? timing?) — needs design in this vault first
6. **Escape route prerequisites → loot placement pass** — make sure the items each route needs actually spawn ([[Loot & Economy]]); **Prison → Setup Items & World Loot** is on `dev`
7. **Career M6+ facility scenes** — State ×3 → Federal ×5 (County stub `CountyJail` + Dev Sandbox already playable)

## Done on `dev`

- **Vault Phase 7 truth sync** — hub / engineering / surface notes match Career+Social (PRs #86–#88+)
- **Social ecosystem v3** — [[Social Ecosystem & Gangs]] / [[Social & Reputation]]
- **Prison career ladder M1–M5** — [[Prison Career Ladder]] (+ [[World Saves & Start Screen]] · [[Facility Transfer & Graduation]])
- **Escape completion** — [[Escape Completion System]]
- **Realistic 13-phase schedule** — [[Time & Schedule]]
- **BlenderKit prison build** — [[Blender Asset Kit]] · [[Editor Tooling]]
- **ProBuilder rebuild pipeline** — [[Editor Tooling]]
