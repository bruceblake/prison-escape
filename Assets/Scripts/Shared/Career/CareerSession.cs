...
namespace Prison.Career
{
    ...
    public static class CareerSession
    {
        ...
        public static void SyncGlobalsFromScene()
        {
            ...
            // Gang reputation tag carries globally; local membership resets per facility
            // (Social Ecosystem & Gangs v3 §10).
            var social = Prison.Social.SocialWorld.Instance;
            if (social != null && social.IsBuilt)
            {
                g.gangId = "";
                g.gangRank = 0;
                for (int gangIdx = 0; gangIdx < Prison.Social.GangCatalog.GangCount; gangIdx++)
                {
                    var gang = Prison.Social.GangCatalog.Get(gangIdx);
                    if (gang == null) continue;
                    if (social.Gangs.IsTraitorLocked(gangIdx))
                    {
                        g.gangId = $"Traitor — {gang.displayName}";
                        g.gangRank = -1;
                        break;
                    }
                    var rank = social.Gangs.GetRank(gangIdx);
                    if (rank != Prison.Social.GangRank.Outsider)
                    {
                        g.gangId = $"ex-{gang.displayName} {rank}";
                        g.gangRank = (int)rank;
                    }
                }
            }
