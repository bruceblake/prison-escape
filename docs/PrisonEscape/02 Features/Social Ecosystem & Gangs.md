...
**Status:** Implemented — Social micro-stack #58–#72 on **dev**. Remaining: **Resources/Social/** assets install + polish ([[Social & Reputation]]).
**System notes:** [[Social & Reputation]] (v3 SocialWorld) · [[Talk Menu & NPC Profile]] · [[Social Dossier — Relationships & Gangs]] · [[Prisoner AI & NPCs]] · [[Guard AI]] · [[Security, Heat & Alerts]] · [[Loot & Economy]] · [[Inventory & Items]]
**Branch:** merged to **dev** (micro-stack #58–#72)
...
## Teardown of v1 (completed on `dev`)
...
## Architecture (shipped — `Assets/Scripts/Shared/Social/`)

`SocialWorld` hub · `NPCIdentity` / traits / `ArchetypeDefinition` · `RelationshipStore` + `RelationshipMath` · Standing bands · `SocialMemory` · `GossipSystem` · `GangDefinition` / `GangManager` · `TradingService` · `FavorService` · `SnitchSystem` · `SocialSimulationTicker` · `SocialInteractionMenu` · `SocialDossierUI`. Deterministic from visit/world seed. Optional SO install: **Tools → Prison → Social → Install Social Assets**.

## Milestones

| Milestone | Status |
|---|---|
| **M1 Foundation** — v1 teardown; identity; RelationshipStore + Standing | ✅ on `dev` |
| **M2 Talking** — Talk Menu; memory; nameplates; Relationships dossier | ✅ |
| **M3 Gangs** — gangs, territory, membership; Gangs dossier | ✅ |
| **M4 Economy** — trade, wallet, bribes, light job | ✅ |
| **M5 Consequences** — favors, gossip, snitch → targeted shakedown | ✅ (heat-eye floor from snitch still polish) |
| **M6 Guards & balance** — archetypes + bribes + tips; simulator | ✅ core; per-guard Trust→detection polish open |
