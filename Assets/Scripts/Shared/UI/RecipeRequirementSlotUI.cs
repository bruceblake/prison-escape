...
public class RecipeRequirementSlotUI : MonoBehaviour
{
    ...
    private static readonly Color IconShortage = new Color(0.5f, 0.5f, 0.5f, 0.8f); // Soft Grey instead of bright red

    private static readonly Color TextSatisfied = new Color(0.4f, 1.0f, 0.4f); // Brighter Green
    private static readonly Color TextShortage = new Color(1.0f, 0.4f, 0.4f); // Brighter Red
    ...
    public void SetIngredient(CraftingIngredient ingredient, PlayerInventory inventory)
    {
        ...
        if (amountLabel != null)
        {
            ...
            amountLabel.fontSize = 18; // Make it a bit larger
        }

        if (backingImage != null)
        {
            // Fully green background for satisfied items
            backingImage.color = satisfied 
                ? new Color(0.1f, 0.9f, 0.1f, 0.85f) 
